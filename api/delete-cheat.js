// POST /api/delete-cheat — supprime un cheat
module.exports = async function handler(req, res) {
    if (req.method !== 'POST') {
        return res.status(405).json({ error: 'Method not allowed' });
    }

    const { adminPassword, cheatId } = req.body || {};
    if (!adminPassword || !cheatId) {
        return res.status(400).json({ error: 'Données manquantes (adminPassword, cheatId)' });
    }
    ...
    try {
        ...
        return res.status(200).json({ success: true, message: 'Cheat supprimé.' });
    } catch (e) {
        console.error('api/delete-cheat:', e.message);
